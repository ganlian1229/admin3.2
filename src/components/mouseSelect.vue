<template>
    <div class="selectParent" ref="selectParentDom">
        <slot v-bind="{ valueList: props.valueList }"></slot>
    </div>
</template>
<script setup>
...
function boxMousedown(event) {
    ...
    if (
        ...
        mouseObj.childrens = element.children;
        console.log('childrens', mouseObj.childrens);
    } else {
...
function boxMousemove(event) {
    ...
    for (let i = 0; i < selList.length; i++) {
        ...
        if (sl > _l && st > _t && selListOffsetLeft < _l + _w && selListOffsetTop < _t + _h) {
            ...
            activeArr.push(i);
            if (props.isHot) {
                if (!props.valueList[i].isAct) {
                    props.valueList[i].isAct = true;
                    emits('update:valueList', props.valueList);
                }
            }
        } else {
            //取消选中
...
function boxMouseup() {
    ...
    if (mouseObj.selDiv) {
        ...
        //  selectSuccess()
        emits('success', activeArr, cancelArr);
    }
